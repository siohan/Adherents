	...
	if($details ==1)//on affiche le détail sans permettre de changer
	{
		...
		while($row = $dbresult->FetchRow())
		{
			...
			else
			{
				...
				$onerow->delete = $this->CreateLink($id, 'fe_delete', $returnid, 'Supprimer ce choix', array("obj"=>"option_belongs", "record_id"=>$row['id'], "id_inscription"=>$id_inscription, "genid"=>$genid));
