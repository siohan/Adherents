	...
	if($dbresult && $dbresult->recordCount() >0)
	{
		...
		if($dbresult2 && $dbresult2->RecordCount()>0)
		{
			...
			while($row = $dbresult2->FetchRow())
			{
				...
				$onerow->view = $this->CreateLink($id, 'default', $returnid,'Détails',array("display"=>"feViewCc", "record_id"=>$row['commande_id'],"commande_number"=>$commande_number,"fournisseur"=>$row['fournisseur']),$warn_message='',$onlyhref='',$inline='true');
		...
		else
		{
			...
			$this->CreateLink($id, 'default', $returnid, 'Ajouter une nouvelle commande', array("record_id"=>$client,"display"=>"add_cc")));
